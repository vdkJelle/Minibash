...
/*   Updated: 2022/11/09 18:11:58 by jelvan-d      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
int	redirection_check_fd(char *s, int fd[2])
{
	...
	{
		print_error(1, make_array(SHELL, s, ": ", strerror(errno)));
		return (1);
	}
	return (0);
}
...
static int	handle_redirection(t_word **arg, int i, int fd[2])
{
	if (!ft_strcmp(arg[i]->word, ">\0"))
	{
		if (fd[WRITE] != NO_REDIRECTION && close(fd[WRITE]) == -1)
			return (redirection_close_error(arg[i + 1]->word));
	...
	{
		if (here_doc(arg[i + 1]->word, fd))
			return (1);
		return (redirection_check_fd("/tmp/here-document", fd));
	}	
	return (redirection_check_fd(arg[i + 1]->word, fd));
}
...
int	handle_redirections(t_expression *expression, t_execute *exec)
{
	int	i;
	...
	while (expression->arg && expression->arg[i])
	{
		...
		{
			if (handle_redirection(expression->arg, i, exec->fd))
				return (1);
			i++;
		}
		i++;
