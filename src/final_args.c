...
** Assumes there is an argument after the redeirection
**		arguments where checked for syntax errors in parser_check
...
/*
** Puts all information needed to execute the command into a null-terminated array of strings and handles redirections
...
**		the redirection and next argument not added to the final array
** No return value
*/
	...
	t_expression *expression, t_execute *exec)
{
	...
	{
		if (is_redirection(expression->arg[i]->word)
			&& expression->arg[i]->metacharacter)
		{
