...
/*
**	Calls the correct word segment handler function
...
static e_special_char	identify_character(char *s, int i)
{
	if (s[i] == '\'' && !(count_backslash(s, i) % 2))
		...
		&& !is_control_operator(s[i + 1]) && !(s[i + 1] != '_'
			&& s[i + 1] != '?' && !ft_isalpha(s[i + 1])))
...
**	Checks if the first character is a metacharacter it is saved as a word ...
**	... to check for a syntax error later
**	Loops over the input string and calls identify_character
**	If the character is a backslash it can be skipped
**	If the character is a metacharacter the end of the word has been ...
**	... reached and the function returns
**	If the character is not a backslash or a metacharacter the correct ...
**	... word_segment_handler function is called
